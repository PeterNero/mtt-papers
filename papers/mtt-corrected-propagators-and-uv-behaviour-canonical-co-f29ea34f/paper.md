...
- $`A`$ is the linearized damping/geometric operator around the coherent fixed point;

- $`P`$ is the coherent-sector projector;

- $`\chi(A)`$ is the admissible spectral acceptance window associated with the gap and truncation regime;
...
# Recovery of the ordinary propagator

<div class="theorem">

**Theorem 3** (Distributional recovery). *Let $`m>0`$, $`d<\infty`$, and define $`\Delta_\tau`$ by <a href="#eq:filtered-propagator" data-reference-type="ref+label" data-reference="eq:filtered-propagator">[eq:filtered-propagator]</a>. Then
``` math
...
*Proof.* For each $`k`$, $`e^{-\tau |k|^2}\to1`$. Moreover,
``` math
0\le
        \frac{e^{-\tau |k|^2}}{|k|^2+m^2}
        \le
...
**Theorem 4** (Finite diagonal). *For every $`d<\infty`$, $`m>0`$, and $`\tau>0`$,
``` math
I_d(m,\tau)<\infty .
```
Moreover,
...
*Remark 5* (Normalization convention). Two closely related filters occur in the literature:
``` math
e^{-\tau A}(A+m^2)^{-1},
        \qquad
        e^{-\tau(A+m^2)}(A+m^2)^{-1}.
...
**Proposition 6** (Low-momentum correction bound). *For $`|k|^2\le R^2`$,
``` math
\left|
...
*Proof.* Use $`0\le 1-e^{-a}\le a`$ for $`a\ge0`$. ◻
...
**Proposition 7** (Bubble finiteness). *For every $`p\in\mathbb R^d`$, $`m>0`$, and $`\tau>0`$,
``` math
B_\tau(p)<\infty .
```*
...
*Proof.* For large $`|k|`$, $`|k+p|^2\ge \frac12|k|^2-|p|^2`$. Hence
``` math
e^{-\tau |k|^2}e^{-\tau |k+p|^2}
        \le
        C_p e^{-c_\tau |k|^2}
...
**Lemma 8** (Loop-momentum quadratic control). *For a connected graph and a choice of independent loop momenta, there exist constants $`c_\Gamma>0`$ and $`C_\Gamma\ge0`$, depending on the graph and external momenta, such that
``` math
...
*Proof.* By the quadratic-control lemma,
``` math
...
# Proper-time representation and positivity
...
# Numerical anchoring from existing data
...
As an illustrative collider-scale anchoring, LHC Run 3 operates at a proton–proton collision energy of $`13.6\,\mathrm{TeV}`$ . Requiring the coherence scale to lie above this scale gives the conservative estimate
``` math
\Lambda_{\mathrm{eff}}\gtrsim 13.6\,\mathrm{TeV},
        \qquad
        \sqrt{\tau}\lesssim 1.45\times 10^{-20}\,\mathrm{m}.
```
A stronger illustrative benchmark comes from contact-interaction/compositeness searches, where global summaries commonly quote lower scales in the multi-TeV to tens-of-TeV range . Taking
``` math
...
```
. A Planck-scale coherence length would correspond to
``` math
...
1.  finite coincident correlators;

2.  modified short-distance potentials;

3.  exponentially softened loop amplitudes;

4.  low-energy corrections of order $`\tau |k|^2`$;
...
B. Simon, *Functional Integration and Quantum Physics*. AMS Chelsea.

J. Glimm and A. Jaffe, *Quantum Physics: A Functional Integral Point of View*. Springer.

V. Rivasseau, *From Perturbative to Constructive Renormalization*. Princeton University Press.

CERN, “The third run of the Large Hadron Collider has successfully started,” CERN News, 5 July 2022.

Particle Data Group, “Searches for Quark and Lepton Compositeness,” Review of Particle Physics, revised August 2025.

NIST CODATA, “CODATA Value: Planck length,” 2022 CODATA recommended values.
