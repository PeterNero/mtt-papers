# Contextual Revision v7: 2026-09-12

Supersedes: v6 as current source. Released metadata identity and the prior audit below are preserved; this is an unpublished local revision, not release approval.

Read the complete current manuscript, not only the Kernel excerpts. The new recorder-source interface cites the contextual Born/record owner and frozen sources for the canonical law, conditional continuum compiler, and ontology non-entailment. Its actual hypotheses and role in transition completion are explicit. Existing kernel, contraction, decoder, OU, and record-stabilization results remain unchanged.
