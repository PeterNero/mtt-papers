...
\begin{description}
\item[Supersedes.] Version~7, the previous authoring revision; version~5
remains the released edition.
\item[Reason.] A contextual citation to a later calculation paper introduced
a reverse reference into the foundational sequence.
\item[Resolution.] Application-specific calculations are removed. The general
distinction between a finite-time transition and a path-limit theorem remains,
with all inputs to the latter stated here. Only earlier FP papers and standard
mathematical literature are used.
\item[Retained result.] The deterministic, stochastic, modal and conditional
homogenization results retain their statements and hypotheses.
\item[Remaining boundary.] An application must establish its driver and the
enhanced invariance principle; these are not supplied by rank or truncation.
\end{description}

\section*{Version 7 Revision Note}
\noindent\textbf{Supersedes.} Version 6, the previous authoring revision;
version 5 remains the released edition.
\textbf{Reason.} The finite-mode calculations now have a separate manuscript
owner and must not be mistaken for a realization of the mixing hypothesis.
\textbf{Resolution.} Adds a scoped companion citation and distinguishes
recurrence, finite-time transition control, and the stochastic-driver limit.
\textbf{Retained result.} The deterministic, stochastic, and conditional
homogenization theorems and their proofs are unchanged.
\textbf{Remaining boundary.} A selected physical noise source and controlled
reduced-dynamics limit are still required.
...
A physical application must independently select the damping generator,
...
We use the FP--I/II control framework \cite{Nero2025FP1,FP2}. The compact internal space
...
\paragraph{What an application must check.}
Neither a finite truncation nor a nonzero transition between two states
verifies the enhanced invariance principle. The theorem below requires a
limit law for an entire family of paths, including their iterated integrals.
An application must therefore establish the fast dynamics, its invariant
measure and the required uniform bounds. This paper supplies the conditional
analytic implication, not an application-specific driver or calculation.
...
Only the earlier FP framework, the locally stated hypotheses and standard
mathematical literature enter the analytic arguments. No later calculation
is a premise or a verification of the homogenization hypotheses. Numerical
applications may test those hypotheses and cite this foundation, not the
reverse.
