...
\begin{description}
\item[Supersedes.] The original \emph{Projection--Admissibility Principle:
Structural Constraints on Effective Physical Description}.
\item[Reason.] Noninjectivity of a cross-level projection was incorrectly used
to rule out a right section and to infer several unrelated physical effects.
\item[Resolution.] Version~2 separates right sections, left decoders,
autonomous descent, and effective mergers, and replaces the obstruction claim
by the correct factor-through and conditioning theorems.
\item[Retained result.] Projection and admissibility remain useful structural
organizers once every map is typed.
\item[Remaining boundary.] Probability, entropy, irreversibility, and physical
recovery require measures, dynamics, and observable-specific hypotheses.
\end{description}

\tableofcontents

\section{How to Read the Principle}

A projection deliberately forgets distinctions.  The central issue is not
whether information was forgotten, but which later questions can still be
answered from what remains.  Four questions recur throughout the paper:
\begin{description}[leftmargin=3.4cm,style=nextline]
\item[Choose a representative.]
Required equation: $T_tS_t=\operatorname{id}$.  Failure means that some final
reduced states have no admissible upper representative, or none with the
requested regularity.
\item[Recover the actual input.]
Required equation: $D_tT_t=\operatorname{id}$.  Failure means that distinct
upper inputs have become indistinguishable at the final reduced level.
\item[Evolve autonomously.]
Required equation: $F_tP_0=P_t\Phi_t$.  Failure means that the initial reduced
state omits distinctions that influence the later reduced state.
\item[Detect a merger.]
Required relation: $F_t(y)=F_t(y')$ for $y\ne y'$.  Its occurrence means that
distinct initial reduced states have the same final reduced image.
\end{description}
The equations point in different directions and therefore cannot be replaced
by one slogan about invertibility.

\subsection{Object picture: what the reduced state remembers}

For the projection $P_0(x,h)=x$, the coordinate $h$ is hidden.  If upper
evolution changes the visible coordinate by an amount depending on $h$, two
upper states represented by the same $x$ can have different visible futures.
Autonomous descent then fails because the reduced state has forgotten
something needed for prediction.  A representative section can still choose,
for example, $h=0$; that choice does not recover the hidden value that was
actually present.

This memory picture is useful across the applications.  Initial
system--environment correlations, eliminated ultraviolet couplings, interior
boundary data, and complementary coherent modes can all act as an $h$-type
variable.  Whether they matter is decided by the factor-through condition,
not by the mere fact that a projection was used.

\subsection{Argument map}

Sections~2--4 type the maps and prove the descent/recovery classification.
Section~5 gives the genuine diameter obstruction.  Sections~6 and 7 treat
stable continuation and chart boundaries.  Sections~8 and 9 explain which
measure and temporal data must be added for stochastic or irreversible
interpretations.  Section~10 proves the separate locality-descent statement.
The final
applications then show how the same distinctions change the interpretation of
open systems, Wilsonian reduction, exterior gravity, and MTT.
...
The load-bearing part of Theorem~\ref{thm:main} is item~3.  It asks whether
the upper evolution maps every initial projection fiber into one final
projection fiber.  If so, all upper representatives of one reduced state give
the same reduced future and $F_t$ is well defined.  If not, choosing a section
can manufacture one representative-dependent trajectory, but it cannot turn
...
Thus coherent compression preserves locality only for the algebra on which the
displayed commutation calculation is legal.  The theorem transports an
existing upper microcausal relation; it does not create spacetime locality from
an arbitrary projector and does not force the compressed state to be
unentangled.
