...
\begin{abstract}
This revised paper separates topology-only obstructions from representation
checks and from physical source theorems. Chiral multiplicity is an index on
...
physical convention is $Y=n/N_0$. The former ``exact hypercharge'' argument is
only a reconstruction after observed charge differences are inserted. A newer
finite-branch theorem is stronger: on the selected chiral carrier, anomaly
