...
\hypersetup{
  pdftitle={A Conditional Operator Encoding of Wave-Particle Duality in MTT},
...
claim that all of them descend from one MTT source for an arbitrary physical
apparatus remains conditional. This does not reopen the separately established
canonical operational q79 model. A proposed additional survivor-basin account
requires its own state-dependent measure intertwiner.
\end{abstract}

\section*{Version 7 Revision Note}
\begin{description}
\item[Supersedes] Version 6. Published release metadata and the previous
revision notes are retained.
\item[Reason] The analytic assumptions needed a current, source-bound
consumer reference to the finite q79 transfer hierarchy and its owner.
\item[Resolution] Adds a contextual comparison of Hodge compression,
nonminimal response transfer, and physical operator selection, citing the
Cohesive owner and the successive frozen finite results. The generic
apparatus boundary is separated from established canonical operational q79.
\item[Retained] Kernel representations, the boundary-correct sharp limit,
Schur channels, ordinary physical instruments, and the explicit basin
criterion remain at their existing tiers. No owner proof is duplicated.
\item[Open boundary] A selected continuum endpoint, domain-preserving
intertwiner, physical normalization, and apparatus-specific source data
are not supplied by the finite transfer hierarchy.
\end{description}

\section*{Version 6 Revision Note}
\begin{description}
\item[Supersedes] Version 5. The previous release remains public until a new release is approved.
\item[Reason] Import the corrected sharp-limit theorem with its boundary domain.
\item[Resolution] Imports the corrected sharp-limit statement and removes its duplicated proof.
...
proposed basin realization assumes re-coherence: high-frequency or inadmissible components are damped, the
...
\subsection{The finite transfer hierarchy and its analytic owner}
\label{subsec:finite-transfer-consumer}
The analytic direction is one-way: the Fixed Points series supplies
abstract existence, stability, and operator hypotheses; downstream MTT
models must instantiate them. The sharp-limit statement below is imported
from the canonical-kernel paper with its selected self-adjoint boundary
realization and smooth domain. Cohesive repair and the finite q79 transfer
hierarchy remain in their designated owner, \emph{Cohesive Closure Repair
and Its Hodge, Kernel and Projection Shadows}, particularly its sections
on finite Weyl completion, response retraction, and all-arity source maps
\cite{MTTFPSpine,MTTKernel,MTTCohesiveTransfer}.

The finite example clarifies what an operator kernel does not determine.
The selected two-direction Weyl--Koszul differential graded algebra has
harmonic dimensions \((1,2,1)\), and transfer to that harmonic exterior
algebra has no higher products. Its constraint projector is not the
zero-mode projector of the completed finite Dirac response; equal ranks
do not identify their ranges \cite{FiniteWeylHodge}.
The symmetric signed-direction completion instead has 144 cochain
coordinates and twelve additional harmonic classes. The old complex is
an isometric reducing cochain summand, but its off-harmonic image is not
product closed. Thus preserving a Laplacian is weaker than preserving
the nonlinear response algebra \cite{SymmetricWeyl}.

The exact response retract retains 48 coordinates: the old 36-dimensional
complex plus those twelve classes. It is not a minimal cohomology model.
Its transferred \(m_3\) corrects the associator of the compressed binary
product, and \(m_4\) is also nonzero. The later two-parity recurrence gives
a nonzero family at every arity; this resolves the older packet's open
nontruncation question without claiming exhaustive tables above arity
four \cite{ResponseMthree,ResponseMfour,HigherTransfer}.
A contraction-preserving differential graded algebra map transports all
operations at once, by the owner theorem's tree argument. The finite
order-36 covariance preserves that complete contraction, so all-arity
equivariance is proved, not extrapolated from a few tests
\cite{AllArityTransfer}.

For the present paper these results are an exact finite compatibility
example and a consumer contract. They do not select the continuum
operator \(A\), its HYM endpoint, the detector instrument, or a physical
action. A continuum application needs the actual source map, reducing
operator domains, metric/product and homotopy compatibility, and error
bounds for any approximate square. The finite 48-dimensional target is
neither the physical rank-102 response bundle nor a spacetime dimension.
None of these later MTT results is imported back into Fixed Points I--VI.

\paragraph{Canonical quantum scope.}
The generic encoding theorem here must not be read as denying the
established canonical operational q79 source. That separate finite-symbol,
binary, one-anchor construction has its selected noncommutative state and
recorder structure, as recorded in the frozen source-base import
\cite{CanonicalQ79Source}. Extending it to every apparatus class considered
here, or identifying an additional survivor-basin measure, is a stronger
physical task. Measurement remains ordinary physical interaction and
recording, not a new principle supplied by a kernel limit.

\paragraph{The output law is already supplied on its canonical domain.}
\label{par:canonical-output-consumer}
The current record-measure owner explains the stopped canonical Fock
output law and its exact second-moment descent \cite{MTTRecordOwner,FockOutput}.
For the declared binary recorder, restricting the normal joint state to
its commuting nondemolition output algebra gives the classical history
measure. Its first-count density for label \(a\) is
\(\gamma e^{-\gamma s}\operatorname{tr}(\rho P_a)\,ds\), and the no-count
effect is \(e^{-\gamma u}I\). Preparations with the same second moment
therefore give the same stopped-output law. This uses accepted normal-state
expectation semantics; it is not a pre-quantum derivation of probability.
The basin criterion later in this paper tests an additional proposed
realization and does not reopen this operational result.

\paragraph{The continuum compiler has a specified input.}
\label{par:continuum-recorder-consumer}
The same owner imports the conditional continuum-to-recorder compiler
\cite{MTTRecordOwner,ContinuumRecorder}. A selected invariant three-mode
sector, kernel/support projectors of ranks one and two, an isometric
projector intertwiner, a common clock rate, and a minimal nondemolition
L\"uders context compile the jump coefficients and Fock instrument.
The clock is not forced by a Hessian eigenvalue. Approximate projector
intertwining has explicit error bounds; conditional states additionally
need event probabilities bounded away from zero. The already closed
Cauchy/Fock continuum must not be confused with the remaining internal
physical HYM endpoint and dynamic finite-sector map. These are the
source obligations behind the generic \(A\) and apparatus assumptions here.

\paragraph{A record measure is not an ontology selector.}
\label{par:ontology-consumer}
The current locality/records account imports the operational non-entailment
result \cite{MTTLocalityOwner,OntologyNonentailment}. The same canonical
record law and conditional states admit a one-record-per-sample completion
and a coactual-support completion. Their operational agreement entails
neither interpretation. This is not a global context-independent value
assignment to every projector, and it neither disproves many-worlds nor
selects an objective single-history dynamics. It is why the instrument and
the optional survivor-basin interpretation remain separate in this paper.
...
\begin{remark}[Imported delta limit of finite coherent kernels]
\label{thm:delta-limit}
The joint sharp-limit theorem is owned by the projected-kernel paper
\cite{MTTKernel}; we import its corrected realization-dependent statement.
Fix a nonnegative self-adjoint elliptic realization \(A\) on compact \(X\),
including its boundary conditions. For \(\tau_N\downarrow0\) and
\(\Lambda_N\to\infty\),
\begin{equation}
  B_{N,\tau_N}f\longrightarrow f
\end{equation}
strongly in \(L^2\) for every \(L^2\) input. For
\(f\in D_\infty(A)=\bigcap_{m\ge0}\operatorname{Dom}(A^m)\), convergence
also holds in every graph norm and hence smoothly. This domain equals
\(C^\infty(X)\) on a closed manifold, but at a boundary it imposes
all-order compatibility. Independently,
\begin{equation}
  K_{N,\tau_N}(x,y)
  \longrightarrow
  \delta(x-y)
\end{equation}
distributionally on the interior product, or on \(X\times X\) for closed
\(X\). These are distinct topologies, not equivalent assertions. The
owning theorem gives the proof. For this paper the consequence is a
controlled sharp representation limit, not a derivation of Born
probabilities, apparatus dynamics, physical source selection, or collapse.
\end{remark}
...
measurement context is ordinary device-dependent physical interaction represented by an
instrument. It may damp the off-diagonal coherences that made interference visible.
Throughout this section and the experimental modules, references to a selected survivor
basin assume the additional instrument--basin intertwiner and capture hypotheses of
Assumption~\ref{ass:physical-reading} and Section~\ref{subsec:minimal-basin-capture}.
They are not consequences of the channel axioms or of the canonical output law.
...
A detector record is operationally specified by its instrument. Under the proposed basin
realization, the device stabilizes one branch of the basin partition. This does not mean it reveals a context-free
...
Assumption~\ref{ass:fixed-point-spectral-data}.
  ...
  Remark~\ref{thm:delta-limit}, the corresponding kernels converge to
...
\end{equation}
in the distributional sense. The sharp delta limit is exactly Remark~\ref{thm:delta-limit}.
\end{proof}
...
from a sharp position effect. By Remark~\ref{thm:delta-limit}, the exact sharp position description is
...
wave-to-particle conversion. Deriving those data for the generic apparatus class remains
open; the canonical operational source in Section~\ref{subsec:finite-transfer-consumer}
is not reopened.
...
\(\mathbb P(i)=\tr(\rho E_i)\), with the canonical output realization already supplied
as explained above. The additional basin-measure proposal seeks a realization of this
pairing, not a replacement for the established operational law.
...
damping/admissibility time. An apparatus-specific source theorem is still required. Branch damping is not an arbitrary
visibility factor. It must define a valid quantum channel. Outcome probabilities are not
identified with damping factors. They are instrument probabilities, equal to basin-measure
weights only under the stated bridge criterion.
...
The earlier measurement papers motivate basin stabilization and its proposed probability
bridge. Their basin interpretation remains subject to the criterion above; it is distinct
from the current canonical output theorem. The present paper adds a structural synthesis:
\begin{center}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item the measurement sequence, with an optional disturbance-stabilized basin realization;
  \item the basin-measure sequence, where the bridge criterion tests agreement with Born weights.
\end{enumerate}
...
The last two arrows require the basin-capture and measure-intertwining assumptions.
They are possible additional layers, not prerequisites for the canonical operational
record law and not a selection of ontology.
...
the standard formalism. This generic encoding does not itself derive that
formalism; the established canonical q79 operational tier is a separate
result, as explained in Section~\ref{subsec:finite-transfer-consumer}.
...
\bibitem{MTTFPSpine}
P. Nero, \emph{Fixed Points I--VI}, current analytic series.
Used here as the upstream analytic foundation, not as a consumer of later
MTT finite-response results.

\bibitem{MTTCohesiveTransfer}
P. Nero, \emph{Cohesive Closure Repair and Its Hodge, Kernel and Projection
Shadows}, v14, 2026, sections ``Covariance completion changes the harmonic
content,'' ``The response retract keeps the missing classes,'' and
``One source map can transport the whole hierarchy.''
Theorem owner for the following finite transfer comparison.

\bibitem{FiniteWeylHodge}
MTT causal-base research, \emph{Finite Weyl--Koszul Hodge calculus and interaction boundary}, 2026.
\href{https://github.com/PeterNero/mtt-results-repro/blob/f141a20ea23c5c3ff19cc2161c0e226e29ade8a7/release/results/causal_base_selected_finite_weyl_koszul_hodge_and_interaction_cutset_packet/artifact.json}{Frozen source artifact}, snapshot f141a20e.

\bibitem{SymmetricWeyl}
MTT causal-base research, \emph{Symmetric signed Weyl calculus and isometric retraction}, 2026.
\href{https://github.com/PeterNero/mtt-results-repro/blob/f141a20ea23c5c3ff19cc2161c0e226e29ade8a7/release/results/causal_base_q79_symmetric_weyl_calculus_isometric_retraction_packet/artifact.json}{Frozen source artifact}, snapshot f141a20e.

\bibitem{ResponseMthree}
MTT causal-base research, \emph{Response retraction and transferred ternary operation}, 2026.
\href{https://github.com/PeterNero/mtt-results-repro/blob/f141a20ea23c5c3ff19cc2161c0e226e29ade8a7/release/results/causal_base_q79_symmetric_response_retraction_transferred_m3_packet/artifact.json}{Frozen source artifact}, snapshot f141a20e.

\bibitem{ResponseMfour}
MTT causal-base research, \emph{Transferred quaternary operation and its identity}, 2026.
\href{https://github.com/PeterNero/mtt-results-repro/blob/f141a20ea23c5c3ff19cc2161c0e226e29ade8a7/release/results/causal_base_q79_symmetric_response_transferred_m4_packet/artifact.json}{Frozen source artifact}, snapshot f141a20e.

\bibitem{HigherTransfer}
MTT causal-base research, \emph{Higher-jet filtration and all-arity nontruncation}, 2026.
\href{https://github.com/PeterNero/mtt-results-repro/blob/f141a20ea23c5c3ff19cc2161c0e226e29ade8a7/release/results/causal_base_q79_higher_transfer_jet_filtration_and_m5_feasibility_packet/artifact.json}{Frozen source artifact}, snapshot f141a20e.

\bibitem{AllArityTransfer}
MTT causal-base research, \emph{All-arity contraction naturality and finite covariance}, 2026.
\href{https://github.com/PeterNero/mtt-results-repro/blob/f141a20ea23c5c3ff19cc2161c0e226e29ade8a7/release/results/causal_base_q79_all_arity_source_promotion_packet/artifact.json}{Frozen source artifact}, snapshot f141a20e.

\bibitem{CanonicalQ79Source}
BEQ Research, \emph{Canonical q79 selected finite source base}, 2026.
\href{https://github.com/PeterNero/mtt-results-repro/blob/f141a20ea23c5c3ff19cc2161c0e226e29ade8a7/release/results/beq_q79_fmo_q79_selected_source_base/artifact.json}{Frozen source-base import}, snapshot f141a20e.

\bibitem{ContinuumRecorder}
P. Nero, \emph{Continuum spectral recorder compiler and intertwiner error}, 2026.
\href{https://github.com/PeterNero/mtt-results-repro/blob/f141a20ea23c5c3ff19cc2161c0e226e29ade8a7/release/results/q79_continuum_recorder_compiler/artifact.json}{Frozen source artifact}, snapshot f141a20e.

\bibitem{OntologyNonentailment}
P. Nero, \emph{Operational ontology non-entailment}, 2026.
\href{https://github.com/PeterNero/mtt-results-repro/blob/f141a20ea23c5c3ff19cc2161c0e226e29ade8a7/release/results/q79_ontology_nonentailment/artifact.json}{Frozen source artifact}, snapshot f141a20e.

\bibitem{FockOutput}
P. Nero, \emph{Canonical q79 Fock output measure and second-moment descent}, 2026.
\href{https://github.com/PeterNero/mtt-results-repro/blob/f141a20ea23c5c3ff19cc2161c0e226e29ade8a7/release/results/q79_fock_output_measure/artifact.json}{Frozen source artifact}, snapshot f141a20e.

\bibitem{MTTRecordOwner}
P. Nero, \emph{Born-Compatible Record Measures and the Classical
Concentration Limit: Separate Theorems and Their MTT Interface}, v3, 2026.
Contextual owner of the canonical output and continuum recorder imports.

\bibitem{MTTLocalityOwner}
...
v2, 2026. Contextual owner of the operational non-entailment import.
