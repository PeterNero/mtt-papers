...
\end{equation}
where \(A\) is the fixed-point stabilization operator, \(P\) is the coherent-sector projector,
\(\chi(A)\) is the admissible spectral window, and \(\ta\) is the damping-selected
proper-time/heat-time scale.
...
Fifth, measurement is treated as finite survivor-basin selection. A detector is represented by
a measurement context \(\Meas\), with its own device-sector data
\begin{equation}
  (A_{\Meas},P_{\Meas},\chi_{\Meas},\tau_{\Meas},
  \{E_i^{(\Meas)}\}_{i\in I},\mathfrak B_{\Meas}).
\end{equation}
Here \(\mathfrak B_{\Meas}=\{B_i^{(\Meas)}\}_{i\in I}\) denotes the survivor-basin partition.
...
\end{equation}
when the exponent defines an admissible positive kernel. The exact detector record is not a
pre-existing context-free point value revealed without disturbance. It is a stabilized downstream
record produced by the device-specific evolve--project--stabilize cycle.
...
The first question is answered by basin-measure structure. In the Born-rule shadow-bridge
program, outcome probabilities are associated with the relative measures of stabilized
selection basins. Schematically,
\begin{equation}
  \mathbb P(i)
  =
...
admissibility-weighted basin measure. This is the probability side of measurement.
...
device, the path branches are separated into distinct survivor basins and the interference term
is suppressed. The outcome is exact within the selected measurement context because the
device stabilizes a definite record, but the context itself is created by the device-specific
disturbance.
...
\subsection{Damping-selected proper-time scale}
\label{subsec:damping-selected-time}

The admissible kernel contains a heat/proper-time parameter. In ordinary regularization this
parameter is often chosen by hand. In the MTT fixed-point setting, it is selected by the damping
required to reduce discarded-sector leakage below an admissibility tolerance.
...
\begin{proposition}[Minimal admissible heat/proper-time]
...
\begin{equation}
  ...
  \log\frac{C_Q}{\epsadm}
\end{equation}
is \(\epsadm\)-admissible. The minimal time selected by this bound is
\begin{equation}
  \ta
  :=
  \lambda_\ast^{-1}
...
\begin{definition}[Admissible coherent operator]
\label{def:admissible-coherent-operator}
Given the fixed-point data of Assumption~\ref{ass:fixed-point-spectral-data} and the damping-selected
scale \(\ta\), define
\begin{equation}
  \Badm
  :=
...
Thus \(K_{\rm det}\) is not an extra arbitrary smoothing function. It is the detector-sector
realization of the same MTT admissible-kernel architecture. Different devices may have
...
\begin{enumerate}[label=(\roman*)]
  ...
  records;
  \item \(\mathfrak B_{\Meas}=\{B_i^{(\Meas)}\}_{i\in I}\) is the corresponding survivor-basin
  partition.
\end{enumerate}
...
First, it has an outcome-selection part. In the basin-measure account, outcome probabilities
are determined by the relative measures of stabilized basins:
\begin{equation}
  \mathbb P(i)
  =
  \frac{\mu(B_i^{(\Meas)})}
...
\end{equation}
But the finite kernel is the native MTT object. The delta/PVM form is the singular
measurement limit.
...
Assumption~\ref{ass:fixed-point-spectral-data}.
  ...
  \item Under the joint sharp limit of complete bandwidth and vanishing proper-time width,
  \(\Kadm(x,y)\) converges to \(\delta(x-y)\) distributionally in the sense of
  \cref{thm:delta-limit}.
\end{enumerate}
\end{theorem}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item stabilized detector records correspond to survivor basins of the measurement context.
\end{enumerate}
\end{assumption}
...
\end{enumerate}
Measurement contexts interpolate between these shadows by damping or preserving
off-diagonal branch coherences and stabilizing survivor-basin records. Therefore wave-like
interference and particle-like localization are not distinct ontological primitives; they are
different projection shadows of one finite coherent-sector excitation.
\end{theorem}
...
is a mixture of branch-stabilized records. The exact record is supplied by survivor-basin
stabilization in the measurement context.

Thus the same coherent excitation gives wave-like or particle-like behavior according to
which projection context is imposed and which coherences survive. This proves the
projection-duality statement.
\end{proof}
...
\begin{corollary}[No primitive wave-to-particle conversion]
...
\begin{proof}
The proof of \cref{thm:wave-particle-projection-duality} shows that wave-like and
particle-like descriptions are downstream shadows of the same coherent-sector excitation.
Measurement changes the effective description by applying finite effects and admissible
branch damping, not by changing the ontological type of the excitation.
\end{proof}

\subsection{Relation to complementarity}
\label{subsec:relation-complementarity}

The projection-duality theorem refines, rather than discards, the traditional complementarity
intuition. Complementarity says that wave and particle descriptions are revealed by different
experimental arrangements. MTT explains why this is so: different experimental arrangements
implement different projection contexts.
...
Thus wave--particle duality is not an additional mystery placed on top of MTT. It is a direct
consequence of coherent projection, finite kernels, phase evolution, and survivor-basin
selection.
...
formal postulates. In MTT, both are downstream limits of finite coherent projection.

\subsection{Born probabilities and basin weights}
\label{subsec:born-basin-weights}

The projection-duality theorem concerns the coexistence of wave-like interference and
particle-like localization. It does not by itself replace the Born rule. In the MTT corpus,
Born probabilities are associated with basin-measure structure.

Let \(\{B_i\}_{i\in I}\) be the survivor-basin partition associated with a measurement context
\(\Meas\), and let \(\mu\) be the relevant admissibility-weighted basin measure. The outcome
probability is
\begin{equation}
  \mathbb P(i)
  =
...
The MTT claim is broader and more structured. Decoherence explains how coherence can
become inaccessible after tracing out environmental degrees of freedom. MTT additionally
requires:
\begin{enumerate}[label=(\roman*)]
...
\end{equation}
The kernel is not chosen as a phenomenological smoothing function. It is selected by
fixed-point coherent data and damping/admissibility. Branch damping is not an arbitrary
...
\section{Compatibility with basin-measure and disturbance-stabilization papers}
...
The basin-measure bridge treats quantum probabilities as shadows of basin measures on an
...
Thus the Born-rule bridge and the projection-duality theorem are complementary. The Born
bridge supplies the basin-measure weights for stabilized outcomes. The duality theorem
supplies the representation structure explaining why coherent interference can coexist with
localized detection.
...
Within this domain, wave-like interference and particle-like localization are not separate
postulates. They follow from the same coherent projection architecture.
...
\begin{equation}
  ...
  =
  (A_{\Meas},P_{\Meas},\chi_{\Meas},\tau_{\Meas},
  \{E_i^{(\Meas)}\},\mathfrak B_{\Meas}).
\end{equation}
Different devices can therefore define different branch bases, different finite detector
effects, different damping matrices, and different survivor-basin partitions. The exact
detector record is exact inside the measurement context because the device stabilizes one
survivor basin.
...
recovered when branch coherences are strongly damped. Thus MTT does not reject the
standard formalism; it supplies a deeper projection architecture explaining why the standard
formalism has both wave-like and particle-like faces.
...
\begin{equation}
  ...
  \Lambda_{ab}^{(\Meas)},\quad \{E_i^{(\Meas)}\},\quad \mathfrak B_{\Meas}.
\end{equation}
...
Nevertheless, the structural conclusion is already fixed. The quantum object is not forced
to be either a wave or a particle. In MTT, both are downstream shadows of one finite coherent
excitation, revealed differently by local probing, spectral phase evolution, and device-specific
