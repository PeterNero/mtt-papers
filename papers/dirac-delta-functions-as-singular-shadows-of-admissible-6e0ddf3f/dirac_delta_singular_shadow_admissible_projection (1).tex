...
\hypersetup{pdftitle={Dirac Delta Functions as Singular Shadows of Admissible Projection}, pdfauthor={Peter Nero}}
...
we separately record the heat-kernel approximate identity. We emphasize that these are
distinct regularization mechanisms: sharp spectral cutoffs are band-limited identity
kernels, while heat/proper-time kernels are positive smoothing kernels. These standard
analytic facts provide the rigorous anchor for the MTT interpretation: the native object
in a finite-capacity coherent regime is not the exact identity kernel \(\delta(x-y)\), but
a bounded sector kernel \(K_{\mathrm{coh}}(x,y)\) induced by a coherent projector
\(\Pi_{\mathrm{coh}}\). The delta is recovered only after an idealizing limit in which the
retained sector is treated as complete or the admissibility kernel is collapsed to zero
width.
...
\[
\boxed{\text{a Dirac delta marks where a finite admissible process has been
idealized as exact zero-width selection.}}
\]
...
The interpretation used below is more careful than the slogan ``a gap gives a delta
width.''  A spectral gap by itself separates retained and discarded modes and gives
damping of the discarded sector.  A finite coherent bandwidth, a finite proper-time
support, or a finite disturbance--damping floor supplies the actual resolution scale.
Together these data imply that the effective identity kernel is finite-resolution rather
than literally point-supported.

Thus the delta limit corresponds to an additional idealization in which coherent
projection is treated as complete and infinitely sharp.  MTT does not begin with this
sharpness.  It begins with bounded projection, finite spectral separation, controlled
truncation, and finite admissibility width.
...
\begin{corollary}[Finite projection kernels are band-limited identity kernels]\label{cor:finite-proj}
...
\(K_\Lambda\) is a band-limited identity kernel, while \(\delta(x-y)\) is the
infinite-bandwidth identity kernel. In general \(K_\Lambda\) need not be positive and need
not be spatially localized in the same manner as a heat kernel; it is a spectral
regularization of the identity, not necessarily a probabilistic smearing kernel.
\end{corollary}
...
\begin{corollary}[MTT reading: sector identity versus full identity]\label{cor:mtt-reading}
...
\end{equation}
is the identity kernel only on the retained coherent sector:
\begin{equation}
\Pi_{\mathrm{coh}}^2=\Pi_{\mathrm{coh}},\qquad
\Pi_{\mathrm{coh}}f=f\quad\text{for }f\in\Ran(\Pi_{\mathrm{coh}}).
\end{equation}
It is therefore a sector identity, not automatically an approximate identity on the full
underlying space. The Dirac delta is recovered only after the additional idealization that
the retained coherent sector is complete for the observables under consideration, or after a
family of coherent projectors is taken to a full-resolution limit.
\end{corollary}

\begin{remark}[Status of the MTT reading]
...
relevant effective theory arises from coherent truncation. This avoids the stronger and
generally false claim that an arbitrary fixed finite projector is itself an approximate
...
\begin{proposition}[Three non-equivalent delta replacements]\label{prop:three-kernels}
There are three distinct kernel types that must not be conflated:
\begin{enumerate}
\item Sharp spectral cutoffs \(K_\Lambda\), which are band-limited identity kernels and
converge distributionally to \(\delta\) as \(\Lambda\to\infty\).
\item Heat/proper-time kernels \(H_\tau\), which are smoothing approximate identities and
converge distributionally to \(\delta\) as \(\tau\downarrow 0\).
\item Fixed coherent-sector kernels \(K_{\mathrm{coh}}\), which act as identity kernels on a
specified admissible sector but need not approximate the identity on the full underlying
space.
\end{enumerate}
Consequently, the phrase ``delta replacement'' has a different mathematical meaning in
each case: spectral truncation, smoothing regularization, or sector identity.
\end{proposition}

\begin{proof}
The first two claims are \cref{thm:spectral-delta,thm:heat}. The third follows from
idempotence of the coherent projector and from the fact that a fixed proper subspace
projection cannot converge to the full identity unless it is embedded in an increasing family
whose ranges become complete. Hence the three kernels share a common singular reference
point, \(\delta\), but represent different finite-resolution structures.
\end{proof}
...
\end{equation}
A finite coherent effect may be modeled by
\begin{equation}
E_x^{\mathrm{coh}}(y,z)
=K_{\mathrm{det}}(y,x)\overline{K_{\mathrm{det}}(z,x)},
\end{equation}
where \(K_{\mathrm{det}}\) is a detector/admissibility kernel. For a complete measurement
one should use a normalized POVM satisfying
\begin{equation}
\int E_x^{\mathrm{coh}}\,\dd x=I
\end{equation}
on the relevant retained sector. The probability becomes
\begin{equation}
...
\[
\boxed{\text{some UV divergences diagnose the use of zero-width coincidence kernels}.}
\]
Renormalization then becomes, at least in part, the downstream repair mechanism required
after over-sharp projection. This does not eliminate renormalization, and it does not imply
that every divergence has this origin. It explains why a large class of renormalization
problems appears exactly where the idealized continuum theory compresses admissible
...
\item M. Reed and B. Simon, \emph{Methods of Modern Mathematical Physics I: Functional Analysis}, Academic Press.
\item M. Reed and B. Simon, \emph{Methods of Modern Mathematical Physics II: Fourier Analysis, Self-Adjointness}, Academic Press.
\item L. Hörmander, \emph{The Analysis of Linear Partial Differential Operators}, Springer.
\item P. B. Gilkey, \emph{Invariance Theory, the Heat Equation, and the Atiyah--Singer Index Theorem}, CRC Press.
\item M. E. Taylor, \emph{Partial Differential Equations I--III}, Springer.
\item L. Schwartz, \emph{Théorie des distributions}, Hermann.
\item L. D. Faddeev and V. N. Popov, ``Feynman diagrams for the Yang--Mills field,'' \emph{Physics Letters B} 25 (1967).
\item V. N. Gribov, ``Quantization of non-Abelian gauge theories,'' \emph{Nuclear Physics B} 139 (1978).
\item C. Becchi, A. Rouet, and R. Stora, ``Renormalization of gauge theories,'' \emph{Annals of Physics} 98 (1976).
\item I. V. Tyutin, ``Gauge invariance in field theory and statistical physics in operator formalism,'' Lebedev preprint (1975).
\item K. G. Wilson and J. Kogut, ``The renormalization group and the epsilon expansion,'' \emph{Physics Reports} 12 (1974).
\item R. Brunetti, K. Fredenhagen, and R. Verch, ``The generally covariant locality principle,'' \emph{Communications in Mathematical Physics} 237 (2003).
\item P. Nero, \emph{Fixed Points I--VI}, Modal Triplet Theory series.
\item P. Nero, \emph{Modal Diagrammatics: The Origin of Feynman Rules from Coherent Modal Geometry}, Modal Triplet Theory series.
\item P. Nero, \emph{Measurement as Disturbance and Stabilization in Modal Triplet Theory}, Modal Triplet Theory series.
\end{enumerate}
