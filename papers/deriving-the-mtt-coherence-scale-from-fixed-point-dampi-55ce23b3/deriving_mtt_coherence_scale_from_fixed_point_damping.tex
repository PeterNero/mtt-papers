...
\title{Deriving the MTT Coherence Scale from Fixed--Point Damping\\
\large Canonical Proper-Time Kernels from Admissibility Thresholds}
\author{Peter Nero}
\date{April 2026}
...
\]
and if admissibility requires residual incoherent leakage below a tolerance \(\varepsilon_{\rm adm}\), then the minimal admissible projection time is not a free regulator but is fixed by
\[
...
\begin{enumerate}
\item A residual incoherent leakage bound determines a minimal admissible projection time \(\tau_{\rm adm}\).
\item This time scale canonically determines the proper-time filter \(e^{-\tau_{\rm adm}A}\).
\item Together with a coherent projector \(P\) and spectral window \(\chi(A)\), this fixes the kernel \(K_{\rm adm}\).
...
\begin{theorem}[Minimal admissible projection time]
Under the incoherent damping bound, any time \(t\) satisfying
\[
t\ge \frac{1}{\lambda_\ast}\log\frac{C_Q}{\varepsilon_{\rm adm}}
\]
is \(\varepsilon_{\rm adm}\)-admissible. The minimal admissible time determined by this bound is therefore
\[
\boxed{
\tau_{\rm adm}
=
...
\begin{remark}[Why this is not a regulator choice]
A heat-kernel regulator normally chooses \(\tau\) as a computational cutoff. Here \(\tau_{\rm adm}\) is fixed by a damping inequality and an admissibility threshold. The scale appears before any divergent integral is considered. It is selected by the fixed-point stabilization mechanism.
\end{remark}
...
\begin{theorem}[Canonical kernel selection]
For fixed regime data
\[
...
\[
\exp\left(-\frac{\tau_{\rm adm}n^2}{R^2}\right)\sim e^{-1}
\quad\Longleftrightarrow\quad
|n|\sim \frac{R}{\sqrt{\tau_{\rm adm}}}.
\]
Therefore
\[
|n|_{\rm eff}
...
\section{Application to Propagator Corrections}
...
\section{Why This Solves the Kernel-Choice Objection}
