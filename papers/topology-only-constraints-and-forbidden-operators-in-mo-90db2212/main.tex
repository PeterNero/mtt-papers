...
\item[Supersedes.]
Version 2, \url{https://doi.org/10.5281/zenodo.21713543}; and the original
Version 1, \url{https://doi.org/10.5281/zenodo.18261774}.
\item[Reason.]
Version 1 contained the mathematical errors corrected below. Version 2 made
those corrections but cited the curated calculation repository only at
repository level; it did not include the hash-bound per-result evidence block
required to reproduce the current finite MTT status rows.
\item[Resolution.]
Version 3 retains the Version 2 mathematics, which separates character
lattices, global gauge-group descent,
determinant-line anomalies, internal Picard classes, coefficient spaces, and
actual overlap pairings. It adds an explicit computational-evidence section
with repository commit, manifest hash, result identifiers, tiers, and the
open strict-upgrade row.
\item[Retained content.]
All Version 2 theorems, corrections, explanatory discussion, and physical
...
Version 3 preserves the mathematical decisions made in Version 2 and adds
the hash-bound evidence record below. The table therefore compares the
original Version 1 statements with the corrected Version 2--3 position.
...
The finite MTT statements in \cref{tab:mtt-status} are status imports, not
proofs of the bundle-selection theorems in this paper. The exact rows support
only the declared finite representation, gauge-group, hypercharge, and
anomaly statements. The profile row records the adopted embedded-SM standard,
and the open row marks the stronger no-knob boundary.

The referenced rows are frozen to the curated results repository state below.
Hashes are grouped in eight-character blocks for line breaking.
\begin{quote}\small
\textbf{Repository:}
...
Tier labels are quoted verbatim from that manifest. A row used directly
supports only its stated finite calculation; it does not select the physical
compactification or prove an operator coefficient.

\paragraph{Rows used directly in this paper.}
\begin{itemize}
\setlength{\itemsep}{0.25em}
\item \path{A46/typed_family_representation}
(\emph{derived exact}).\par
48-state family-diagonal chiral representation and anomaly table.
\item \path{A47/native_gauge_group}
(\emph{derived exact}).\par
Native \(U(1)\), \(SU(2)\), \(SU(3)\) automorphisms and \(\Z_6\) quotient.
\item \path{A50/neutral_summand_hypercharge}
(\emph{derived exact}).\par
Neutral-summand completion and unique anomaly-free shared hypercharge circle.
\item \path{A22/e6_qpsi_qcd_anomaly}
(\emph{derived exact}).\par
\(E_6\) \(Q_\psi\) matter/exotic QCD anomaly cancellation audit.
\end{itemize}

\paragraph{Corpus-status cross-check.}
\begin{itemize}
\setlength{\itemsep}{0.25em}
\item \path{A04/final_12_of_12_audit}
(\emph{profile replay}).\par
Twelve-obligation embedded renormalized-SM equivalence audit at the declared
profile standard.
\end{itemize}
...
\begin{itemize}
\setlength{\itemsep}{0.25em}
\item \path{A05/strict_upgrade_ledger}
(\emph{open}).\par
