...
- Peter Nero
current_version: v2
date: Version 2, 12 September 2026 Unpublished interpretive manuscript
generated_from_main_tex_sha256: 2e3add5531a1a585b2797f524abe9513664bd28ad46bc798c2977ef9ec4609ac
...
Peter Nero. Version 2, 12 September 2026 Unpublished interpretive manuscript
...
**Supersedes:** Version 1.0 as the current unpublished manuscript; its revision note and author-approval requirement are retained.

**Reason:** The ontology non-entailment source requires an explicit account of the operational data retained by its countermodels.

**Resolution:** The frozen source is explained through its signature, finite checkpoint, selective states, basis change, and ensemble ambiguity. Its local consistency result is separated from a physical actualization law.

**Retained:** The ordinary-instrument treatment of measurement, scoped canonical output law, free locality results, and illustrative experiments.

**Open boundary:** No global hidden-variable model, many-worlds refutation, universal apparatus source, or objective actualization dynamics is selected. The local-first ontology remains an interpretive proposal for author review.
...
<a id="sec:ontology-nonentailment"></a>
...
The frozen non-entailment source makes the logical scope explicit \[[29](#ref-NeroOntologyFrozen)\]. Its operational signature contains the Hilbert space, state, observable algebra, channel, dilation, and instrument. It contains no predicate saying that every nonzero coefficient is an actual world, or that precisely one history is actual. Adding either interpretation without changing the retained data cannot be decided from that data alone. The finite partition above makes this absence constructive: each atom has one record label, and the corresponding ordinary conditional output state can be attached to it. Averaging these state-valued outputs returns the same instrument as the unitary realization. This is more than matching three bare numbers, but remains a construction for the declared context.

Two further examples prevent overreading it. A normalized vector has one nonzero coefficient in a basis containing it and three after the selected three-dimensional Fourier change of basis. Coefficient counting is therefore not an invariant count of actual worlds. Likewise, a mixed state has both a basis-vector ensemble and a superposition ensemble; the identical density operator fixes the same instrument outputs without selecting one underlying ensemble as ontology. Neither example identifies all coherent alternatives as merely subjective: their interference and detector couplings remain ordinary physical predictions.

This source does not assign simultaneous values to incompatible projectors, derive an apparatus-independent sample space, or produce settings-independent Bell-factorized responses. It therefore does not evade Bell or contextuality constraints by relabeling a finite sample space. The single-record model establishes non-entailment of many actual worlds; it does not establish entailment of the proposed single-world process law. The source’s conditional marked-Poisson construction adds quadratic hazards and a clock as hypotheses, not a selected universal actualization mechanism. These distinctions are the paper’s principal use of the result: the interpretation can use its operational consistency without claiming source selection it does not have.
...
<a id="ref-NeroOntologyFrozen"></a>

\[29\] P. Nero, *q79 ontology non-entailment*, frozen contextual source (2026). <https://github.com/PeterNero/mtt-results-repro/blob/f141a20ea23c5c3ff19cc2161c0e226e29ade8a7/release/results/q79_ontology_nonentailment/artifact.json>.
