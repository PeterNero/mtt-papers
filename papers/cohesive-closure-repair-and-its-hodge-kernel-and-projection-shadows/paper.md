...
- main.bib
current_version: v15
date: September 2026, Version 15
generated_from_main_tex_sha256: 9ee41091420551aa37b1d40ee05ba628b3d29b0219facb667a198680a98975a5
...
# Version 15 Revision Note

Supersedes
The local Version 14 manuscript; no public release is implied.

Reason
The complete mathematical-language atlas needed contextual integration as an evolving source, not an undated list of open targets.

Resolution
Explains the certified representation diagram, its exact finite compatibility operator and the distinctions between tangent directions, moduli, physical parameters and obstruction certificates.

Retained result
All sixty-seven prior integrations retain their scopes; later source authority takes precedence over historical checkpoints. The foundational Fixed Points papers do not depend on this paper.

Remaining boundary
A typed diagram does not select a physical point, positive metric, signed action, quantum measure or global analytic lift.
...
Supersedes
The local Version 10 manuscript; no public release is implied.

Reason
The repair/action boundary needed the established anomaly-to-string constructor, Bott–Chern deformation lift and constrained moment-map roles.

Resolution
Connects these three results, distinguishes holomorphic from unit-normalized volume forms, and retains the pseudo-Kahler qualification. An adjoint Lie-algebra inclusion is not an associative-algebra inclusion.

Retained result
All forty-five earlier source integrations survive. The ambient six-row compiler is retained with its volume-form roles clarified; a constrained Hessian is not an ambient spectral intertwiner.

Remaining boundary
A complex anomaly primitive does not select a real physical flux, a positive Calabi metric, a common endpoint or its action. The foundational Fixed Points series is unchanged.

# Version 10 Revision Note

Supersedes
The local Version 9 manuscript; no public release is implied.

Reason
The BV/topology boundary needed the already-available first-order matter and normalized spectral-localization results.

Resolution
Explains graded index, chain projectors, mirror and exotic-rank tests, the ordinary graph-Prym obstruction, and the exact 85-by-82 evaluation criterion with its derived cohomology shift.

Retained result
All forty-one earlier source imports and the foundational Fixed Points series retain their stated scopes.

Remaining boundary
The selected line and its evaluation entries, harmonic normalization and physical lifting operator are not supplied by an auxiliary rank witness. Equivariant duality retains its canonical-line data.
...
Supersedes
The local Version 6 manuscript; no public release is implied.

Reason
The inner-charge discussion and finite transfer results needed their nested Witten and noncentral coefficient constructions explained together.

Resolution
Integrates six local records from outer nilpotence through restricted contraction to a positive curved coefficient algebra. Distinguishes the self-adjoint coefficient matrix from the full module superconnection, cyclic from positive pairings, and minimal from nonminimal transfer.

Retained result
Previous residual, projection, first-jet, charge and finite-response results remain at their stated scopes. Fixed Points I–VI are unchanged.

Remaining boundary
The finite tensor model does not select a physical factorization, global shared-line realization, analytic domain, action or value.
...
Thus a stationary diffusion limit needs an independently justified recurrent fast driver and a mixing/scaling argument, a continuum or thermodynamic limit, or a declared open system. Toral-chaos examples remain valid examples of separately supplied drivers; they are not flows of a strictly decreasing repair cost. These distinctions preserve the repair Hessian theorem and locate the extra dynamical input precisely. The stationary-ensemble argument belongs to Fixed Points III. The separate finite-spatial-mode theorem and its certified model calculations belong to the finite-mode companion ; their proofs are not duplicated here. Its improved cubic heat-trace bounds and phase-sensitive L11 channel interval are examples for a supplied Hamiltonian and preparation, not inputs to the present abstract repair-Hessian theorem or selected physical noise amplitudes.
...
## Inner curvature and an outer nilpotent differential

Let $`M`$ be a smooth Riemannian configuration manifold with a smooth internal charge $`Q_x`$, defect $`\Phi(x)=Q_x^2`$, and a declared positive fiber metric. Put $`V=\frac12\left\lVert \Phi \right\rVert^2`$, allowing an explicitly normalized trace in the finite examples below. On forms over $`M`$, define
``` math
d_{\beta,V}=e^{-\beta V}d_M e^{\beta V}
             =d_M+\beta\,dV\wedge,\qquad \beta\geq0.
```
This is Witten deformation . Conjugation gives $`d_{\beta,V}^2=0`$ for every smooth $`V`$, whether or not the internal defect vanishes . Thus $`[Q_x,-]^2=[\Phi(x),-]`$ can be nonzero while the outer differential is nilpotent. These operators act on different spaces; neither identity repairs the other by changing its meaning.

For a closed compact $`M`$, the usual elliptic realization gives the positive operator
``` math
H_{\beta,V}=d_{\beta,V}d_{\beta,V}^{\dagger}
            +d_{\beta,V}^{\dagger}d_{\beta,V}
```
and its contractive heat semigroup. Boundary or noncompact settings need compatible adjoints and self-adjoint domains; the polynomial computations below are formal differential-operator identities, not domain certificates. On the configuration line let $`A_V=\partial_x+\beta V'`$. Then
``` math
\begin{align*}
...
\end{align*}
```
At a critical point, $`V''`$ contains the full residual-Hessian term from <a href="#thm:residual-hessian" data-reference-type="ref+label" data-reference="thm:residual-hessian">1</a>, not just the Gram term. For $`\Phi(x)=1+x^2`$, one has $`V=\frac12(1+x^2)^2`$ and, at zero,
``` math
\Phi=1,\quad \Phi'=0,\quad V'=0,\qquad
 |\Phi'|^2=0,\quad \Phi\Phi''=2,\quad V''=2.
```
The opposite potential terms $`-2\beta`$ and $`2\beta`$ are not opposite spectral-positivity claims: positivity belongs to the factorized differential operators, including their kinetic terms.

This construction retains the defect’s full second jet, but it does not make the Witten operator equal to the internal curvature or to the finite-dimensional Hessian matrix. On a closed compact $`M`$, multiplication by $`e^{\beta V}`$ identifies its cohomology with de Rham cohomology. Nilpotence alone therefore does not select zero defect or a unique physical configuration. The supplied $`\beta`$ is a localization parameter, not Lorentzian time or a derived thermodynamic temperature.

## What the eliminated pair leaves behind

For the auxiliary algebra $`\Lambda(a,b,c)`$ with $`dc=ab`$, the six harmonic representatives are
``` math
H=\operatorname{span}(1,a,b,ac,bc,abc).
```
Let $`i`$ include them, let $`p`$ remove the pair $`c,ab`$, and set $`h(ab)=c`$, with $`h=0`$ otherwise. Then
``` math
pi=I_H,\quad dh+hd=I-ip,\quad h^2=ph=hi=0.
```
Under the transfer convention already used in <a href="#sec:response-retract" data-reference-type="ref+label" data-reference="sec:response-retract">8.5</a>, the two nonzero examples are
``` math
m_3(a,a,b)=ac,\qquad m_3(a,b,b)=-bc
 \quad\cite{PreprojectionNilTransfer2026}.
```
They remember the removed relation $`dc=ab`$; a cohomology-dimension count does not encode them. Minimal-model and strict-unital constructions are standard homological algebra .

With weights $`w(a)=w(b)=1`$, $`w(c)=2`$, every source operation and contraction map preserves total integer weight, hence also its reduction modulo three. This is a representation-level phase statement, not a construction of the continuum shared line or an identification of Heisenberg/Nil with the global q79 manifold.

The top-form functional $`\tau_A(u)=[abc]u`$ is graded cyclic but is not a positive Hilbert norm. It also carries weight four: under the stated circle action, $`\tau_A(g_\theta u)=e^{4i\theta}\tau_A(u)`$. Thus its pairing is covariant with that character, not automatically an invariant scalar pairing. An invariant scalar interpretation needs the inverse-character line or equivalent compensating data. Preserving a cyclic contraction and preserving weights are compatible assertions; neither supplies that additional scalar trivialization.

## Which data a nested contraction must preserve

A quasi-isomorphism is insufficient for this purpose. The restricted transfer record requires the same admissible base, typing and boundary data; maps $`p,i,h`$ satisfying the contraction and side identities; first-order and adjoint intertwiners; compatible cyclic and Hilbert pairings; a unit and equal normalized repair potentials; and shared-line equivariance . The maps must be parallel over the configuration base, with a common $`\beta`$, to intertwine the outer Witten operators. Transfer trees also need a terminating filtration or a proved convergence domain.

For two algebraically compatible contractions the composite is
``` math
p=p_2p_1,\qquad i=i_1i_2,\qquad h=h_1+i_1h_2p_1.
```
These formulas preserve the normalized contraction identities. Analytic admissibility of the composite still needs a common transfer-control class; convergence on unrelated domains is not automatically enough. Likewise, local arrows alone do not prove effective overlap descent.

Here is the same-source central example. Let $`\Gamma_A`$ be form parity, take $`S=\mathbb R^{2|2}`$, and define
``` math
...
```
Tensor the preceding contraction with $`I_4`$. It intertwines $`Q`$, its declared adjoint and curvature. Rank-normalized trace gives $`V=\frac12(1+x^2)^2`$ on both fibers; raw traces would differ with rank. Because $`p`$ is independent of $`x`$, it also intertwines the formal outer Witten blocks. The same $`p,i,h`$ produce the nonzero $`m_3`$.

Strict intertwining and transfer remain different claims: the source DGA has no ternary operation to project, while its minimal model has one. Equality of normalized costs is verified for this source, not inferred from any arbitrary rank-reducing map.

## Two ways to retain noncentral curvature

The infinitesimal route works over $`D=\mathbb Q[\epsilon]/(\epsilon^2)`$, with matrix indices starting at zero and parities $`(0,1,0)`$. Put
``` math
q=E_{10}+\epsilon E_{21},\qquad
 F=q^2=\epsilon E_{20},\qquad [F,E_{00}]=F\ne0.
```
For homogeneous $`X`$, the graded commutator $`d_qX=qX-(-1)^{|X|}Xq`$ satisfies $`d_q^2X=[F,X]`$ and $`d_qF=0`$. Every two-curvature product $`FXF`$ vanishes over $`D`$. For the Heisenberg contraction and $`\delta=\Gamma_A\otimes q`$,
``` math
(h\delta)^2=(\delta h)^2=0,\qquad
 (I-h\delta)^{-1}=I+h\delta.
```
This is exact algebraic termination, not a small numerical error . The $`24`$-to-$`18`$ module ranks are over $`D`$, hence double over $`\mathbb Q`$. The coefficient-of-$`\epsilon`$ supertrace is nondegenerate and graded cyclic, not positive. No nonzero nilpotent self-adjoint operator can represent this $`F`$ on a positive finite Hilbert space.
...
```
Here $`q=q^{\mathsf T}`$, $`F=q^{\mathsf T}q\geq0`$, and $`[F(0),E_{12}]=E_{12}`$. The same form contraction gives $`Q_A=d\otimes I_3+\Gamma_A\otimes q`$ on 24 module directions and $`Q_H=\Gamma_H\otimes q`$ on 18. It retains $`Q_A^2=I_A\otimes F`$, adjoint intertwining and the normalized cost
``` math
V(x)=\tfrac13(1+x^2)^2,\qquad
 V''(0)=\tfrac13\left\lVert F'(0) \right\rVert_{\mathrm{HS}}^2+
         \tfrac13\langle F(0),F''(0)\rangle_{\mathrm{HS}}
       =\tfrac23+\tfrac23=\tfrac43
 \quad\cite{PreprojectionPositiveNested2026}.
```
The omitted half would be a genuine error in a Gram-only Hessian. The positive norm uses ordinary trace; graded cyclicity uses supertrace. They are separate pairings, not one automatically positive cyclic form.

A wording correction is necessary here. The self-adjoint coefficient matrix is $`q`$, not the full module superconnection:
``` math
Q_A-Q_A^\dagger=(d-d^\dagger)\otimes I_3\ne0.
```
Indeed $`d(c)=ab`$ while $`d^\dagger(c)=0`$ in the monomial metric. The positive square $`Q_A^2=I_A\otimes F`$ and all displayed intertwiners remain exact, but $`Q_A^2`$ must not be renamed $`Q_A^\dagger Q_A`$. Neither this module dimension nor the central example’s outer-form dimension identifies the different CBF response carrier, the physical 27-carrier, or the rank-102 bundle.

## Curvature as retained coefficients

The positive curvature is not nilpotent. Nevertheless the strict-unital extension record closes the local curvature-insertion problem for this particular tensor construction . Keep $`B=\operatorname{End}(\mathbb R^{1|2})[x]`$, with $`d_B=[q,-]`$ graded and $`d_B^2=[F,-]`$, as the coefficient algebra. Transfer only the Heisenberg form factor.

For this six-class minimal model, $`m_n=0`$ for $`n\geq4`$. A nonzero internal homotopy introduces $`c`$; a later homotopy cannot remove it to recover $`ab`$, and two branches carrying $`c`$ multiply to zero. Hence every tree with at least two homotopies vanishes. This is specific to the displayed exterior algebra and contraction, not a universal termination theorem.

Use total $`\mathbb Z_2`$ grading for the supermatrix extension; the integer form grading alone does not give an integer grading of the finite matrices. On $`K=H\otimes B`$, the source convention is
``` math
\begin{align*}
 M_0&=-1_H\otimes F,\\
...
\end{align*}
```
The ordered coefficient product is essential; $`B`$ is not commutative. At fixed $`x`$, $`K`$ has dimension $`6\cdot9=54`$, an operator algebra carrier rather than the 18-dimensional module above.

The curved arity-one identity reads
``` math
M_1^2(X)+M_2(M_0,X)-M_2(X,M_0)=0.
```
Thus $`M_0=-F`$ is a sign convention, not a claim that positive curvature became a negative repair norm. In every higher curvature insertion the $`H`$ factor is the strict unit, so $`M_n(\ldots,M_0,\ldots)=0`$ for $`n\geq3`$. The remaining identities reduce to Bianchi, the graded derivation rule and the Heisenberg $`m_2/m_3`$ identities through arity five. No term remains at higher arity. The frozen record checks the 20,736 and 248,832 form/parity cases at arities four and five; those tables are not rerun here.
...
# From anomaly data to a constrained repair problem

There is a useful intermediate answer to the action question. The anomaly equation need not be an unrelated extra row appended to a bundle complex. It can help define the complex itself. Three existing source results make this precise: a string-algebroid constructor, a Bott–Chern lift of bundle deformations, and a moment-map interpretation of the remaining equations . Their order matters. Constructing the complex does not solve its moment-map equations, and solving a form equation over real or complex coefficients does not select an integral flux sector.

## The rank-102 carrier acquires structure

Let $`P`$ be the holomorphic product principal bundle for the tangent, visible and projective hidden factors, and let $`c`$ be a specified nondegenerate invariant bilinear pairing on its Lie algebra. Given a compatible connection $`\theta`$ and a form $`H\in\Omega^{3,0}\oplus\Omega^{2,1}`$ satisfying
``` math
F_\theta^{0,2}=0,\qquad dH+c(F_\theta\wedge F_\theta)=0,
 \tag{S1}\label{eq:string-anomaly}
```
the standard string-algebroid construction supplies the holomorphic Courant structure, including its anchor, pairing and bracket . It fits into
``` math
0\longrightarrow T^*X\longrightarrow Q\longrightarrow A_P
 \longrightarrow0,\qquad
 0\longrightarrow\operatorname{ad}P\longrightarrow A_P\longrightarrow TX
 \longrightarrow0.
```
Its smooth splitting has complex rank $`3+8+8+80+3=102`$. The finite central quotient of the hidden group leaves its rank-80 adjoint honest. It does not trivialize the hidden fundamental gerbe. Thus rank matching identifies the carrier type; the anomaly triple supplies its operations. Once that triple is given, a bracket is not another free choice.

The q79 promotion uses more than an invariant-form computation. The full real map $`H^{2,2}_{BC}(X)\to H^4_{\rm dR}(X;\mathbb R)`$ is injective on the specified q79 geometry. For supplied holomorphic product bundles with the required total characteristic cancellation, choose Chern connections. Their real, closed, pure-type form $`\Xi=c(F_\theta\wedge F_\theta)`$ is de Rham exact. Consequently one may write, in the source’s convention,
``` math
\Xi=2i\partial\bar\partial\tau,\qquad
 H=2i\partial\tau,\qquad dH=-\Xi .
 \tag{S2}\label{eq:string-bc-primitive}
```
The sign follows from $`\bar\partial\partial=-\partial\bar\partial`$. This is a conditional existence construction for the complex anomaly data, not a numerical solution of the HYM equations. Changing the primitive can change the string extension class. Nor is this complex truncated form automatically the real torsion $`H_{\mathbb R}=d^c\omega`$: reality, trace coefficients, integral refinements and the tangent-connection convention must still match. Existing existential hidden-bundle/HYM results are not reopened by these additional requirements on a common physical source.

A small exact witness checks the mechanism, not the continuum existence premises. Take $`\partial\tau=h`$, $`\bar\partial\tau=k`$, $`\partial k=v`$, $`\bar\partial h=-v`$, with all further maps zero. Three closed curvature labels with squares $`2v`$, weighted by $`(3/2,-1/2,-1/2)`$, give $`\Xi=v`$. Then $`H=h`$ solves $`dH+\Xi=0`$. This rational form complex verifies the transgression sign and product-gauge accounting; it is not a selected curvature table.

## What the Bott–Chern lift actually lifts

The form lane remains visible in the deformation algebra. Write $`\mathfrak g^k=\Omega^{0,k}(\operatorname{ad}P)`$ and $`\mathfrak F^k=\bigoplus_{j=0}^k\Omega^{j+2,k-j}`$, omitting types outside the dimension of $`X`$. In nonnegative degrees the string differential graded Lie algebra $`\mathfrak L_Q`$ is an extension of $`\mathfrak g`$ by this abelian form complex. In the constructor’s convention,
``` math
d_Q(a,b)=\bigl(\bar\partial_\theta a,
             db-2c(\partial_\theta a\wedge F_\theta)\bigr).
```
The bracket also has a quadratic form component. In particular, for a bundle Maurer–Cartan solution $`\bar\partial_\theta a+\tfrac12[a,a]=0`$, define
``` math
\Delta(a)=2c(\partial_\theta a\wedge F_\theta)
                 +c(\partial_\theta a\wedge\partial_\theta a).
 \tag{S3}\label{eq:string-deformation}
```
The full equation is $`db=\Delta(a)`$, and the deformed anomaly triple is $`(\theta+a,H-b)`$, not $`(\theta+a,H+b)`$. On the fixed smooth bundle, $`\Delta(a)`$ is closed, pure $`(2,2)`$ and de Rham exact. Complexifying the full q79 Bott–Chern injectivity result gives $`\Delta(a)=2i\partial\bar\partial\tau_a`$; hence $`b=-2i\partial\tau_a`$ solves it exactly. The connection $`\theta+a`$ need not be the Chern connection of the old Hermitian metric.

An auxiliary Hermitian elliptic solution operator $`\mathcal K_{BC}`$ can select a primitive on this exact-form subspace. It defines a section of Maurer–Cartan *sets*,
``` math
s_{\mathcal K}(a)=(a,\mathcal K_{BC}\Delta(a)),\qquad
 \pi s_{\mathcal K}(a)=a.
 \tag{S4}\label{eq:string-mc-section}
```
For each $`a`$, its unquotiented fiber is affine over the closed degree-one form lane. The auxiliary metric chooses a representative, not a physical flux. The section is generally nonlinear, and gauge-equivariance, integral refinement or an $`L_\infty`$ equivalence do not follow from $`\pi s=1`$. The representable product-adjoint subcomplex embeds as a Lie dg algebra in the commutator algebra of cohesive endomorphisms. It is not an associative dg subalgebra: traceless matrices are closed under commutators, but $`E_{12}E_{21}=E_{11}`$ has nonzero trace. Arbitrary off-diagonal or higher cohesive deformations are not covered by this lift.

The finite source example makes the distinction tangible. In degree one use $`(e,b,z)`$, in degree two $`(c,v)`$, and set
``` math
de=c-2v,\quad db=v,\quad dz=0,\quad [e,e]=2c-2v,
```
with all other brackets and degree-two differentials zero. Its Maurer–Cartan residual at $`xe+yb+\lambda z`$ is
``` math
(x+x^2)c+(y-2x-x^2)v.
```
Thus $`s(x)=(x,2x+x^2,0)`$ lifts precisely the two base roots $`x=0,-1`$, and $`\lambda z`$ remains free. The polynomial extension obeys $`s(2)-2s(1)=2b`$, so it is not a linear splitting. A source section has been constructed; uniqueness of a complete physical deformation has not.

## One moment map, with distinct volume-form roles

On a compact real form, the integrable positive horizontal-lift space has the Calabi moment-map description. For fixed positive volume $`\mu`$, put $`f_\omega=\tfrac12\log((\omega^n/n!)/\mu)`$. Its zero locus consists of
``` math
F_\theta\wedge\omega^{n-1}=0,\qquad
 d(e^{-\ell f_\omega}\omega^{n-1})=0,
```
together with the integrability equations $`F^{0,2}=0`$ and $`dd^c\omega+c(F\wedge F)=0`$. For $`n=3`$, $`\ell=1`$, and the holomorphic volume convention, $`e^{-f_\omega}=\|\Omega_{\rm hol}\|_\omega`$. This recovers the Hull–Strominger equations . For the MTT product group, the three HYM rows and the balanced row are components of one moment map; the integrability and anomaly equations instead define its configuration stratum. Matching the selected physical trace and real torsion conventions is required for this interpretation.

The normalization row has a different role again. If $`\mu_{\Omega_{\rm hol}}`$ is the positive volume of the holomorphic form, define
``` math
...
```
This is a coordinate identity. In general $`d\Omega_{\rm unit}=e^{f_\omega}df_\omega\wedge\Omega_{\rm hol}\ne0`$. Requiring the same form to be both holomorphic and unit normalized adds a constant-norm restriction. The conformally balanced equation must use $`\Omega_{\rm hol}`$; replacing it by the unit representative would wrongly force $`d\omega^2=0`$. Thus the two uses of the symbol $`\Omega`$ in an ambient residual compiler must be separated before physical use.

## Restrict the Hessian, not the ambient proof

Suppose an actual stratum inclusion $`\iota:\mathcal Z\to\mathcal C`$ implements the constructor, normalization chart and same-complex identification just described. At an exact solution, let $`j=D\iota`$. If $`\mathcal A\circ\iota=\mathcal N\circ\iota=0`$, then
``` math
D\mathcal A\,j=D\mathcal N\,j=0,\qquad
 \operatorname{Hess}(\mathcal{C}\circ\iota)
...
=\iota_{\rho_\xi}\varpi`$ and $`g(u,v)=\varpi(u,Iv)`$, one has $`\operatorname{grad}_g\langle\boldsymbol\mu,\xi\rangle=I\rho_\xi`$ where the metric is nondegenerate. This ties the moment-map components to infinitesimal complexified gauge motion. The rational witness on $`\mathbb R^4_x\oplus\mathbb R^4_y`$ has standard $`\varpi,I`$, $`g=I_8`$, $`\rho_i=-\partial_{y_i}`$ and $`\mu_i=x_i`$. Its orbit Jacobian is $`I_4`$. It illustrates a variational anchor but does not establish positivity in the geometric moduli problem.

In particular, the cited Calabi construction is generally *pseudo-Kahler*, possibly degenerate, not automatically a positive repair metric. A positive horizontal lift means that its underlying Hermitian form is positive; it does not give positivity of every field-space metric. The anomaly pairing has signed factor weights. Nondegeneracy, positive directions, convexity and coercive estimates need their own hypotheses; at degeneracy an inverse-metric gradient is not available. Neither a local orbit-potential argument nor the finite positive witness establishes global minimization or a Lorentzian physical action.

The later cyclic-cotangent result is already incorporated in the action discussion: the uncompleted string form lane does not automatically inherit a nondegenerate cyclic trace, whereas a shifted cotangent completion supplies dual variables. Shared-line compatibility on the neutral product-adjoint sector likewise does not identify every fundamental charged field. These three imports therefore remove independent bracket and form-lift searches on their stated domain, organize the residual roles, and feed the existing BV construction. They do not select the common physical endpoint, the remaining flux data or the four-dimensional action.
...
The preceding topology obstruction makes bundle-valued matter a natural next construction, but a background deformation and a particle field do not have the same job. Four source results connect the first-order index, associated representation, spectral localization and mirror test . Their contribution here is an ordered calculation, not a new derivation of the already-established finite Standard Model representation.

## Keep the grading before taking the square

Let $`D_+:\mathcal H_+\to\mathcal H_-`$ be a closed densely defined Fredholm operator with its Hilbert adjoint. With their natural domains,
``` math
H_+=D_+^\dagger D_+,\qquad H_-=D_+D_+^\dagger,\qquad
 \operatorname{ind}D_+=\dim\ker H_+-\dim\ker H_- .
```
The nonzero spectra pair; the kernels need not. Replacing $`D_+`$ by its adjoint reverses the index and exchanges the two positive blocks. Consequently the *ungraded* direct sum $`H_+\oplus H_-`$ does not determine the sign of chirality. This is not a claim that a positive operator loses its index when its grading has actually been retained.

For an exact finite example take $`D_+=(1,0,0,0):\mathbb Q^4\to\mathbb Q`$. Its index is $`3`$; the adjoint has index $`-3`$. The full normal matrices have diagonals
``` math
(1,0,0,0,1),\qquad (1,1,0,0,0),
```
related by a permutation. With $`\Gamma=+1`$ on the source and $`-1`$ on the target, however,
``` math
\operatorname{Tr}\!\left(\Gamma f(H_+\oplus H_-)\right)
 =\operatorname{ind}D_+,\qquad f(0)=1.
```
This identity holds for every function on the finite spectrum. In an infinite-dimensional realization the trace must exist; for heat kernels this is the graded heat-trace/index mechanism . Compact ellipticity supplies the usual trace-class setting. A general noncompact Fredholm operator alone does not.

If a finite abelian unitary group action commutes with $`D_+`$, the same argument works in every character block, giving
``` math
k_{+,\chi}+k_{-,\chi}\geq |I_\chi|,
 \qquad I_\chi=k_{+,\chi}-k_{-,\chi}.
```
An inverse-character pairing requires the *equivariant* duality data. Serre duality pairs $`F`$ with $`F^\vee\otimes K_X`$; $`K_X\simeq\mathcal O_X`$ as a line bundle does not by itself identify their group actions. For an equivariantly self-dual, dual-complete carrier with an invariant holomorphic volume form, the familiar relation is $`I_{\chi^{-1}}=-I_\chi`$. If the canonical line has a nontrivial character, that character must be included in the pairing. The source’s finite inverse-character witness uses the invariant-volume case; it does not select the physical q79 character assignment.

On a compact complex threefold with $`c_1(TX)=c_1(F)=0`$, Hirzebruch–Riemann–Roch gives
``` math
\chi(X,F)=\int_X\operatorname{ch}_3(F)
          =\frac12\int_Xc_3(F).
```
Thus a valid bundle with $`|\int_Xc_3(F)|=6`$ has net index magnitude three. This is a difference of multiplicities, not their separate values. By duality $`\chi(X,\operatorname{End}_0F)=0`$ when $`K_X`$ is trivial; the tangent and cotangent contributions also cancel. These facts explain why the rank-$`102`$ background deformation carrier is not itself the associated chiral matter complex. Index zero does not mean invertibility.

## A representation projector is not a zero-mode count

The existing representation construction supplies $`27=16_1\oplus10_{-2}\oplus1_4`$. The $`16`$ here includes the neutral right-handed-neutrino slot of the adopted carrier; it is not a claim that the minimal Standard Model requires this additional field. Let $`Q_\psi`$ have the three indicated eigenvalues. Its polynomial projectors are
``` math
\begin{align*}
...
\end{align*}
```
They sum to the identity, are pairwise orthogonal, and have ranks $`16,10,1`$. If the associated differential has the form $`D_V\otimes I_{27}`$, or commutes with a parallel $`Q_\psi`$, they are chain projectors. A merely pointwise branching is insufficient when the differential mixes those sectors. The choice of $`Q_\psi`$ here does not establish a surviving physical current or require a Wilson line in the independent direct MTT gauge-group construction.

Write $`k_+-k_-=3`$ and $`m=k_-`$. The associated $`16`$ and conjugate lanes then contain
``` math
16(k_++k_-)=48+32m
```
component zero modes. The net count is $`48`$, but the total is $`48`$ only if the opposite kernel vanishes. Each mirror pair adds $`32`$ components without changing the index. The accepted $`48`$-dimensional representation and anomaly calculation remain valid at their original tier; this condition asks whether one particular geometric operator realizes exactly that spectrum.

## What a one-doublet lifting witness does establish

Under the three-copy, no-mirror premise, the remaining $`3(10\oplus1)`$ sector has dimension $`33`$. In each copy its Standard Model representations have dimensions $`3,3,2,2,1`$, denoted $`D,D^c,H_u,H_d,S`$, with hypercharges $`-1/3,1/3,1/2,-1/2,0`$. A complex-linear, gauge-equivariant Hermitian normal acts through the multiplicity spaces:
``` math
H_{\mathrm{ex}}
 = (M_D\otimes I_3)\oplus(M_{D^c}\otimes I_3)
 \oplus(M_{H_u}\otimes I_2)\oplus(M_{H_d}\otimes I_2)\oplus M_S .
```
Each $`M_s`$ is a positive semidefinite $`3\times3`$ matrix. Ranks $`(3,3,2,3,3)`$ give
``` math
\operatorname{rank}H_{\mathrm{ex}}=9+9+4+6+3=31,\qquad
 \dim\ker H_{\mathrm{ex}}=2.
```
The exact witness takes all blocks equal to $`I_3`$, except $`M_{H_u}=\operatorname{diag}(0,1,1)`$. Its kernel carries one positive-hypercharge weak doublet. In contrast, $`m_H I_3`$ has either zero or three family-kernel directions, never one.

This is an equivariant operator feasibility result. It neither selects a physical mass scale nor turns a fermionic matter component into a bosonic Higgs. Field statistics, conjugation and the signed action must supply that interpretation. Nor may every such positive normal be identified with an arbitrary conjugate-pair mass term: a single square $`3\times3`$ map $`B`$ pairing two conjugate families has normals $`B^\dagger B`$ and $`BB^\dagger`$ of equal rank, so cannot alone realize the unequal doublet ranks $`2`$ and $`3`$. Additional bosonic terms, symmetry breaking or another explicitly typed field construction must justify the proposed physical realization. The unit positive gap of the witness is only its chosen normalization.

## Localizing the family question on a curve

The family multiplicity can be tested before normalizing harmonic wavefunctions. For this route take a holomorphic principal elliptic bundle $`\pi:X\to B`$, with translation structure group and relative Jacobian $`J=B\times E^\vee`$. The zero section is $`\sigma:B\to J`$, with image $`D_0`$. Use the normalized BHT analytic gerbe $`\alpha`$, its specified zero-section trivialization $`t_0`$, and the dual normalized universal kernel $`\mathcal P^\vee`$. The underlying twisted transform is established in ; spectral cohomology localization also has the classical bundle-theoretic precedent .

Let $`i:C\hookrightarrow J`$ be a smooth surface finite flat over $`B`$ carrying an $`\alpha|_C`$-twisted line $`\mathcal L_\alpha`$. Assume a smooth transverse intersection $`\Sigma=C\cap D_0`$. The trivialization $`t_0`$ makes the restriction an ordinary line $`M`$ on $`\Sigma`$. In the normalized convention of the source result,
``` math
\begin{align}
 R\pi_*V&=s_*M[-1],&
...
The shift has content. Locally the elliptic transform of the normalized Poincare kernel is $`\mathcal O_0[-1]`$. A derived fiber at the origin has two adjacent classes because the skyscraper’s Cartier resolution acquires a zero differential there. Transverse intersection eliminates the extra Tor terms when the spectral surface is inserted. Principal translations and the specified normalization supply the global zero-section identity without an unprinted base line. A differently normalized kernel, nonprincipal fibration or nontransverse support requires its own formula; it is not covered by <a href="#eq:matter-bht-localization" data-reference-type="eqref" data-reference="eq:matter-bht-localization">[eq:matter-bht-localization]</a>.
...
```
The Euler characteristic changes sign. This upgrades the earlier unordered localization contract to an exact oriented statement on the declared domain, without asserting that its selected physical inputs have already been supplied.

## The ordinary representative fails, but the selected line decides

For the frozen eta9 graph-Prym route, the product Jacobian has $`H^2=2`$, $`D_0^2=0`$, and spectral class $`9H+3D_0`$. Consequently the smooth matter curve has class $`9H`$ on the K3,
``` math
g(\Sigma)=1+\frac{(9H)^2}{2}=82,\qquad \deg(H|_\Sigma)=18.
```
The three graph degrees $`3,3,6`$ cut a length-$`12`$ cycle $`Z`$; the ordinary restricted line is
``` math
M_{\mathrm{alg}}=\mathcal O_\Sigma(5H)(-Z),\qquad
 \deg M_{\mathrm{alg}}=78,\qquad \chi(M_{\mathrm{alg}})=-3.
```
The K3 restriction sequence from $`\mathcal O_B(-4H)`$ to $`\mathcal O_B(5H)`$, with Kodaira/Serre vanishing, gives $`h^0(\Sigma,\mathcal O_\Sigma(5H))=27`$. Twelve point conditions remove at most twelve sections. Hence
``` math
h^0(M_{\mathrm{alg}})\geq15,\qquad
 h^1(M_{\mathrm{alg}})\geq18.
```
Under the localization hypotheses the corresponding component count is at least $`16(15+18)=528`$, not $`48`$. This rejects the *uncorrected ordinary representative* as a mirror-free source. It does not construct that representative as an admissible twisted endpoint or exclude the completed twisted construction.
...
```
Here $`F_\alpha`$ compares an explicit cover trivialization $`b_C`$, available after the cover obstruction vanishes, with $`t_0`$ on the intersection. Degree zero follows after enforcing the fixed Chern/index rows in the same normalization. Zero obstruction is an existence condition, not the printed cochain or its comparison line. Ambient zero-section normalization does not force $`F_\alpha`$ to be trivial.

For smooth ample $`C\subset B\times E^\vee`$, weak Lefschetz identifies $`\operatorname{Pic}^0(C)`$ with the elliptic-factor Picard variety. Those lines restrict trivially to $`\Sigma\subset B\times\{0\}`$. Thus the ordinary fixed-Chern ambiguity on the cover is not a knob that freely tunes the matter line. This conclusion uses ampleness; changing that domain would require a fresh restriction calculation.

## A finite test, not a substitute matrix

Mirror freedom on this route is the exact condition $`h^0(M_\ast)=0`$. The effective locus $`W^0_{78}(\Sigma)\subset\operatorname{Pic}^{78}(\Sigma)`$ has dimension at most $`78`$, while the Picard variety has dimension $`82`$. Generic degree-zero twists therefore avoid it, but this says nothing about where the selected $`F_\alpha`$ lands. The *theta test* here means this line-cohomology non-speciality test; it is not a weak-angle prediction or a reuse of observed flavor data.
...
```
Full column rank, certified by one nonzero $`82\times82`$ minor, is therefore equivalent to mirror freedom. A rank deficiency $`m=82-r`$ gives $`48+32m`$ components. An identity block padded with three rows shows algebraic feasibility, not the value of this geometric map. Its entries must be evaluations of sections of the same selected line, with bases, divisor and exactness certificate.

For the $`c_3(V)=+6`$ normalization, the derived shift fixes
``` math
\chi(X,V)=3,\qquad (h^1(X,V),h^2(X,V))=(0,3)
```
when the test passes. Dualization swaps the middle lanes; the naming of matter and antimatter follows the representation convention. This is not a new selection of a time arrow. Dimensions do not require a metric, but normalized harmonic representatives and their Yukawa overlaps do. The physical exotic/Higgs operator remains a separate same-source export.

The resulting order is now concrete: selected cover and twisted line, normalized restriction and transversality, evaluation-map rank, then harmonic normalization and physical lifting. The localization theorem and finite test are available. A completed obstruction calculation must still emit its trivialization and line before it can instantiate them. This organization preserves the existing finite SM/profile results while identifying what connects them to this geometric source.
...
The full-residual record improves the schematic notation in <a href="#eq:seven-lane" data-reference-type="eqref" data-reference="eq:seven-lane">[eq:seven-lane]</a> to an explicit differential-operator compiler. Write $`\nu=\|\Omega_{\rm hol}\|_\omega`$, and reserve $`c_{\rm vol}`$ for the volume normalization constant, not a third Chern class. Let $`\Psi`$ denote the independently varied volume-form representative in the ambient compiler; it is not silently identified with $`\Omega_{\rm hol}`$. The six extra rows are
``` math
\begin{align}
...
| Anomaly triple to string algebroid | Exact constructor | Supplied holomorphic bundles, pairing and anomaly data |
| q79 Bott–Chern MC lift | Exact on stated sublocus | Nonlinear section, not a full cohesive equivalence |
| Calabi role reduction | Exact conditional | Matched conventions; pseudo-Kahler is not automatically positive |
...
|:---|:---|:---|
| Covariant residual Hessian and exact-zero normal square | This paper | Downstream encodings may cite; foundational Fixed Points stay independent |
...
# A common reference language without losing the source

The mathematical-language atlas records an extended search for a common preprojection description . Its useful proposal is not that every physical object is the same matrix. It is that arithmetic, differential geometry, chain complexes, spectral operators and observables should be *compatible readouts of one specified source*. Different readouts deliberately forget different information. A finite character cannot recover an integral lift; cohomology forgets an acyclic determinant; a normal square forgets the signed first-order operator.

This section imports the atlas’s organizing contribution, not a new proof of each result collected there. The concrete repair, descent, transfer and q79 statements have their contextual homes in the preceding sections and the companion papers. The atlas is an evolving research notebook: its early inventories and successive *next* targets are dated checkpoints, not simultaneous assertions of the present frontier.

## Compatibility has an exact finite meaning

For a finite directed diagram, give each vertex a finite-dimensional Hilbert space $`H_v`$, and each edge $`e:u\to v`$ a specified linear map $`T_e:H_u\to H_v`$. Put
``` math
H_V=\bigoplus_v H_v,\qquad H_E=\bigoplus_{e:u\to v}H_v,\qquad
 (\delta x)_e=T_ex_u-x_v .
```
A compatible tuple is exactly an element of $`\ker\delta`$. With a positive-definite edge metric $`W`$, the compatibility operator is
``` math
\begin{equation}
 L_D=\delta^\dagger W\delta,\qquad
 \ker L_D=\ker\delta,\qquad
 \Pi_D=I-\delta^\dagger(\delta\delta^\dagger)^+\delta .
 \tag{R1}\label{eq:atlas-compatibility}
\end{equation}
```
The last expression is the orthogonal projector for the declared vertex inner product; $`+`$ denotes the Moore–Penrose inverse. Positivity gives the kernel identity, and a singular-value decomposition gives the projector formula. A merely semidefinite $`W`$ could hide discrepancies, so strict positivity matters.

For example, three scalar charts with identity transport around a cycle have $`L_D=3I-\mathbf1\mathbf1^T`$, $`\Pi_D=\mathbf1\mathbf1^T/3`$ and spectrum $`0,3,3`$. There is one common value, not three independent local values. This is the finite linear counterpart of the earlier Cech discussion, not a tensor-product multiplication of dimensions. Adding a specified holonomy can reduce that compatible subspace further.

If the smallest positive eigenvalue is $`\lambda_+>0`$, then
``` math
\begin{equation}
 \|(I-\Pi_D)x\|
 \leq \lambda_+^{-1/2}\|W^{1/2}\delta x\| .
 \tag{R2}\label{eq:atlas-gap}
\end{equation}
```
This is a conditioning estimate in these norms, not a physical mass gap or a selected Hull–Strominger contraction constant. Rescaling the edge metric rescales $`\lambda_+`$. To use an infinite-dimensional analogue one needs closed operators, common domains and closed-range or suitable coercivity hypotheses; the finite pseudoinverse cannot be used unboundedly without them.

## A tangent direction is not automatically a parameter

The atlas informally associates the compatibility kernel modulo gauge with unresolved source directions. The precise local object is
``` math
H^1_{\rm def}=\ker(D\Phi_{s_*})/
                  \operatorname{im}(d_{\rm gauge}),
 \qquad D\Phi_{s_*}\,d_{\rm gauge}=0 .
```
This is a deformation tangent space. Its dimension counts actual nearby moduli only under additional regularity, integrability and quotient hypotheses. The scalar equation $`\Phi(x)=x^2=0`$ is a simple warning: $`D\Phi_0=0`$ has a one-dimensional kernel, although its real zero set is the single point $`0`$. Higher obstructions can remove tangent directions. Conversely discrete branches need not appear as tangent parameters at all. A gauge stabilizer also makes a naive subtraction of dimensions unreliable.

Nor does every modulus become an independent measured coupling. Source selection, boundary conditions, normalization and the observable map still matter. A rank-zero large-gauge kernel, a 33-dimensional interpolation family and a three-source dependency contract therefore answer different questions. They are not competing parameter counts. This keeps the adopted shared-primitive/profile result intact without asserting a zero-input prediction.

A nonzero residual at a trial point is likewise not a no-go theorem. For a linear affine system $`\delta x=b`$, an exact component of $`b`$ orthogonal to $`\operatorname{ran}\delta`$ certifies inconsistency. For a nonlinear problem one instead needs an exclusion bound over the declared domain. Failure of a search, one nonzero $`\beta_C`$, or a singular Jacobian cannot exclude every source in the corpus.

## One source can have several inequivalent shadows

The useful common object is a diagram with explicitly typed maps and compatibility certificates. In a derived or stack-valued setting one may propose its homotopy limit. That notation alone constructs neither a physical point nor a positive Hilbert metric, action or measure. Ordinary equalizers suffice only when higher overlap data can legitimately be omitted. Gerbe-twisted descent, gauge stabilizers and higher products are reasons not to omit them here.

Three recurring examples explain what the arrows must retain. First, a pointed differential determinant or norm fiber includes the trivialization, not just the vanishing Chern class. The faithful $`S(U(3)\times U(2))`$ carrier packages the charge rule without requiring a universally available charge-one sixth-root line. Its representation theorem belongs to the SM encoding; it does not select the internal HYM background.

Second, one contraction can emit higher tree interactions and a complement determinant. Keeping only its projected state space loses the latter. The Hodge/BV and graph papers consequently need the same differential, pairing and eliminated block, not merely matching dimensions. Classical or finite BV identities do not supply a renormalized continuum measure.

Third, the flat shared differential line and a Cox grading are distinct actions. On the truncated label algebra
``` math
\mathbb C[u,v,s]/(u,v,s)^3
```
the degrees have dimensions $`1,3,6`$, and the quadratic labels split as $`s^2`$, $`\{us,vs\}`$, $`\{u^2,uv,v^2\}`$. These give the familiar $`1+2+3`$ filtration at this operator-label tier. The grading acts with weights $`0,1,2`$; a common scalar flat line acts uniformly. Tensoring the two preserves both, but does not identify them. In particular, the grading halfturn has signs $`+,-,+`$, whereas a scalar sign line acts by the same sign on all degrees. Neither construction makes ten ordinary spacetime dimensions or identifies compact phase with time.

## Why the chronology must remain visible

Several apparent contradictions in the notebook are successive answers to different questions. A rank-one Hartshorne–Serre witness first showed a $`3\to7`$ harmonic jump; the later arbitrary-block calculation gives $`9-2k`$, $`k=\operatorname{rank}A`$, and does not select $`k=1`$ physically. The older 19-block augmented-Hodge pattern is retained as a skeleton; the complete residual permits 25 blocks, with the $`30+8\cdot9`$ character reduction only under its explicit neutral-external hypothesis. Later projective hidden-bundle and existential HYM results supersede historical hidden-HYM-open sentences without supplying the entire common physical endpoint.

The primitive eta9 graph route similarly progresses from curve arithmetic to marked maps, interpolation, an explicit equation, good-fiber reductions and characteristic-zero structural promotion. A number-field coefficient lift is not automatically a lift of all its marked graphs. An associated-graded Higgs field is not the full flat Gauss–Manin connection. The notebook’s partial transform-column counts are superseded by the later accepted finite calculations, not reopened as current work. Neither finite return coefficients nor a local chart certificate alone give integral monodromy, global path control or an analytic Deligne zero.

For the analytic question, this paper retains its earlier qualifications on quotient versus vanishing lattices and on Neron-model applicability. A characteristic-zero rank, rational edge class or auxiliary pencil cannot silently change the relevant integral lattice or physical spectral class. The detailed source-by-source review accompanying this version records these chronological distinctions; current authority is not inferred from the earliest status word in a frozen artifact.

## What would make the language successful?

The reference language earns its place when one specified object generates several previously independent readouts, or when a typed comparison rejects a false identification. The common shared-line construction, determinant-fiber charges, support-forced transgression, and common-source tree/loop calculation are examples at their stated tiers. They reduce duplicate work without declaring different geometries, operators or empirical inputs interchangeable.

The remaining physical task is consequently not to draw more arrows. It is to supply the selected geometry/action, analytic spectral comparison and compatible four-dimensional quantum construction specified in the frontier section. The language tells us which outputs then follow and which choices it still does not contain.
