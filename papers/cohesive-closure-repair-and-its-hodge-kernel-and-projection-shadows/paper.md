...
generated_from_main_tex_sha256: 406a4f6285c52328783a7a231f06a70c9ae4e208d1ed3eb9fe013feab69d6a30
...
\end{equation}
```
where the Green identity is taken on the corresponding harmonic complements. If an isometry $`T`$ intertwines two residuals, their metrics, and their backgrounds, then it transports the full nonlinear cost jet, Hessian, heat semigroup, Green operator, spectral projectors, and admitted kernel functional calculus.*

</div>

<div class="proof">

*Proof.* The first three identities are consequences of the spectral theorem for a reducing subspace. For the residual statement, suppose $`\Phi'(Ta)=U\Phi(a)`$ and the residual pairings are carried by the isometry $`U`$. Differentiation transports every residual derivative. The cost identity $`\mathcal{C}'(Ta)=\mathcal{C}(a)`$ transports every cost derivative. At the quadratic level this gives $`H'T=TH`$, after which the spectral theorem gives the remaining functional-calculus identities. ◻
...
1.  *a metric configuration space with a twice differentiable defect section $`\Phi`$ and an exact-zero background $`p_*`$;*
...
\end{equation}
```
The nonlinear flow, tangent calculus, and existing basin limits descend. Compatible projections and isometric intertwiners transport every object in <a href="#eq:master-chain" data-reference-type="eqref" data-reference="eq:master-chain">[eq:master-chain]</a>.*
...
Each record declares its theorem tier, source artifacts, verifier, zero-fit status, and nonpromotion guards. The exact witnesses use rational or symbolic arithmetic where stated. They do not select the physical q79 endpoint or supply measured values. The source TeX, generated Markdown, compiled PDF, metadata hashes, and version-provenance audit accompany this paper in the public MTT papers repository. The evidence snapshot used for Version 1 is commit `ba0874012246bb930677ebfa6f0a6eb33a57a8f0`.
